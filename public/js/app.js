$(document).ready(function () {
    ...
    expense_table = $('#expense_table').DataTable({
        ...
        columns: [
            ...
            { data: 'approved_status', name: 'approved_status', orderable: false }
        ],
    ...
    $(document).on('click', '.expense-approve-btn', function (e) {
        // $('.expense-approve-btn').on('click',function(){
        console.log('asdasdas');
        var trans_id = $(this).data('expense_id');
        $.ajax({
            method: 'POST',
            url: '/approve-expense',
            dataType: 'json',
    ...
    //date filter for fixed assets table
    if ($('#fixedasset_date_range').length == 1) {
        $('#fixedasset_date_range').daterangepicker(
            dateRangeSettings,
            function (start, end) {
                $('#fixedasset_date_range').val(
                    start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
                );
                fixedasset_table.ajax.reload();
            }
        );

        $('#fixedasset_date_range').on('cancel.daterangepicker', function (ev, picker) {
            $('#product_sr_date_filter').val('');
            fixedasset_table.ajax.reload();
        });
    }

    // For Fixed Assets Table
    fixedasset_table = $('#fixedasset_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[1, 'desc']],
        ajax: {
            ...
            data: function (d) {
                ...
                d.expense_category_id = $('select#expense_category_id').val();
                d.expense_sub_category_id = $('select#expense_sub_category_id_filter').val();
                d.payment_status = $('select#fixedasset_payment_status').val();
                d.start_date = $('input#fixedasset_date_range')
                    .data('daterangepicker')
                    .startDate.format('YYYY-MM-DD');
                d.end_date = $('input#fixedasset_date_range')
                    .data('daterangepicker')
                    .endDate.format('YYYY-MM-DD');
            },
        },
        ...
        fnDrawCallback: function (row, data, start, end, display) {
            var expense_total = sum_table_col($('#fixedasset_table'), 'final-total');
            var total_due = sum_table_col($('#fixedasset_table'), 'payment_due');

            $('.footer_expense_total').html(__currency_trans_from_en(expense_total));
            $('.footer_total_due').html(__currency_trans_from_en(total_due));

            $('.footer_payment_status_count').html(
                __sum_status_html($('#fixedasset_table'), 'payment-status')
            );
        },
        createdRow: function (row, data, dataIndex) {
            $(row)
    ...
    // For slection of fixedassets
    $('select#location_id, select#expense_for, select#expense_contact_filter, \
            select#expense_category_id, select#fixedasset_payment_status, \
            select#expense_sub_category_id_filter').on(
        'change',
        function () {
            fixedasset_table.ajax.reload();
