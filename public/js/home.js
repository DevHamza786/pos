$(document).ready(function() {
    if ($('#dashboard_date_filter').length == 1) {
        ...
        let starting = moment().startOf('month');
        let ending = moment().endOf('month');
        update_statistics(starting.format('YYYY-MM-DD'), ending.format('YYYY-MM-DD'));
        $('#dashboard_date_filter span').html(
            starting.format(moment_date_format) + ' ~ ' + ending.format(moment_date_format)
        );
    }
