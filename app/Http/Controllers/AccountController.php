    ...
    public function show($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ...
                ->addColumn('debit', function ($row) use ($account) {
                    // if('CAPITAL/EQUITY')
                    if ($account->account_type->name == 'CAPITAL/EQUITY') {
                        if ($row->type == 'credit') {
                ...
                ->addColumn('credit', function ($row) use ($account) {
                    if ($account->account_type->name == 'CAPITAL/EQUITY') {
                        if ($row->type == 'debit') {
                            return '<span class="debit" data-orig-value="' . $row->amount . '">' . $this->commonUtil->num_f($row->amount, true) . '</span>';
                        }
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ...
                ->addColumn('debit', function ($row) {
                    if ($row->type == 'credit' && $row->account_id != 24) {
                        return '<span class="debit" data-orig-value="'.$row->amount.'">'.$this->commonUtil->num_f($row->amount,true).'</span>';
                    }
                })
                ->addColumn('credit', function ($row) {
                    if ($row->type == 'debit') {
                        return '<span class="credit" data-orig-value="'.$row->amount.'">'.$this->commonUtil->num_f($row->amount,true).'</span>';
                    }elseif($row->type == 'credit' && $row->account_id == 24){
                        return '<span class="credit" data-orig-value="'.$row->amount.'">'.$this->commonUtil->num_f($row->amount,true).'</span>';
                    }
                })  
