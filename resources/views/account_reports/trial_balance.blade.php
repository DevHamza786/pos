            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    for (var key in capital_account_details) {
                        if (capital_account_details.hasOwnProperty(key)) {
                            var accnt_bal = __currency_trans_from_en(capital_account_details[key]);
                            var accnt_bal_with_sym = __currency_trans_from_en(capital_account_details[key],
                                true);
                            var debit_or_credit_class = accnt_bal < 0 ? 'credit' : 'debit';
                            var account_tr = '<tr><td class="pl-20-td">' + key +
                                ':</td><td><input type="hidden" class="' + debit_or_credit_class + '" value="' +
                                Math.abs(accnt_bal) + '">' + accnt_bal_with_sym + '</td><td>&nbsp;</td></tr>';
